export function laconToJson(text: string): string {
...
function processComplexLine(line: string, scope: any, vars: Record<string, string>) {
    let plusIndex = -1;
    let inQuotes = false;
...
function appendValue(target: any, key: string, value: string, vars: Record<string, string>) {
...
function parseInlinePairs(text: string, target: any, vars: Record<string, string>, overwrite: boolean) {
...
function parseValue(val: string, vars: Record<string, string>): any {
    val = val.trim();
